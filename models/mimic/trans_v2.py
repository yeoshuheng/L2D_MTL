...
class TransformerLayer(nn.Module):
    def __init__(self, feature_size, heads=4, dropout=0.3, num_layers=10):
...
class Transformer(BaseModel):
    ...
    def __init__(
        ...
        self.classifier = nn.Sequential(
            nn.Linear(len(self.feature_keys) * self.embedding_dim, 128),  # First hidden layer
    ...
    def forward(self, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        results = {
            ...
            'label_class': label_class.squeeze(),
            'label_reg': label_reg
        }
...
class Transformer_rejector(BaseModel):
    def __init__(
            self,
            dataset: SampleEHRDataset,
            feature_keys: List[str],
        ...
        super(Transformer_rejector, self).__init__(
            dataset=dataset,
            feature_keys=feature_keys,
            label_key=label_key,
            mode=mode,
        ...
        for feature_key in feature_keys:
            self.transformer[feature_key] = TransformerLayer(
                feature_size=embedding_dim, dropout=self.dropout, **kwargs
            )
        output_size = self.get_output_size(self.label_tokenizer)
        # transformer's output feature size is still embedding_dim
        self.rejector = nn.Sequential(
            nn.Linear(len(self.feature_keys) * self.embedding_dim, 128),  # First hidden layer
            nn.ReLU(),  # Activation function
            nn.Dropout(self.dropout),  # Optional: Dropout layer to prevent overfitting

            nn.Linear(128, 64),  # Second hidden layer
            nn.ReLU(),  # Activation function
            nn.Dropout(self.dropout),  # Optional: Another Dropout layer

            nn.Linear(64, self.number_agents)  # Output layer
        )
